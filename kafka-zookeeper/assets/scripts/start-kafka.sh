...
# * ADVERTISED_HOST: the external ip for the container, e.g. `docker-machine ip \`docker-machine active\``. Port will always 9092.
...
if [ ! -z "$LOG_RETENTION_MINUTES" ]; then
    echo "log retention minutes $LOG_RETENTION_MINUTES"
